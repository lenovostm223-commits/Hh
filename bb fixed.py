...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  ADVANCED ARCHIVE EXTRACTION BOT v8.0 - ALL-IN-ONE FINAL EDITION            ║
║  Single file · No bot_design.py needed · All fixes applied                  ║
║  Platforms: Gofile | PixelDrain | Mega.nz | MediaFire | Direct HTTP         ║
║  Extract:   CC | Cookies (per-domain) | Tokens | ULP | Combos               ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class Design:
    """All UI text templates for the bot."""
    ...
    def welcome(user_name: str, user_id: int, trial_tasks: int) -> str:
        return (
            f"{_box('ULTIMATE BOT v8.0', ROCKET)}\n"
...
API_TOKEN = '7233103554:AAGOhoHr5UBGcfouNqGZMlDTOyGLtTWUGBY'
OWNER_ID = 8394758671

# Supabase
SUPABASE_URL = "https://qminvnddokkstzegbqlo.supabase.co"
SUPABASE_KEY = "sb_publishable_Nu9il4qJ7KBz4hmmGdbphg_9l-rKLIl"
...
# ══════════════════════════════════════════════════════════════════════════════
#                       GOFILE DOWNLOADER - FULLY WORKING
# ══════════════════════════════════════════════════════════════════════════════

class GofileDownloader:
    """Working Gofile downloader with proper API"""
    ...
    def collect_files(self, data: Dict, base_path: Path, files: List[Dict]):
        """Collect all files from folder structure"""
        if data['type'] == 'file':
            files.append({
                'name': data['name'],
                'link': data['link'],
            ...
            for i, file_info in enumerate(files, 1):
                if self.progress_callback:
                    asyncio.create_task(self.progress_callback(
                        f"File {i}/{len(files)}: {file_info['name']}",
                        0, 0, 0, file_info['size']
                    ))
...
# ══════════════════════════════════════════════════════════════════════════════
#                       PIXELDRAIN DOWNLOADER - WORKING
# ══════════════════════════════════════════════════════════════════════════════

class PixelDrainDownloader:
    """Working PixelDrain downloader"""
...
# ══════════════════════════════════════════════════════════════════════════════
#                       MEGA.NZ DOWNLOADER - WORKING
# ══════════════════════════════════════════════════════════════════════════════

class MegaDownloader:
    """Working Mega.nz downloader using mega.py"""
    
    def __init__(self, download_dir: Path, progress_callback=None):
        self.download_dir = download_dir
        self.progress_callback = progress_callback
        self.mega = Mega() if HAS_MEGA else None
    
    def download(self, url: str, session_id: str = None) -> Tuple[bool, List[Path], str]:
        """Download from Mega"""
        if not self.mega:
            return False, [], "Mega.py not installed"
        ...
        try:
            ...
            try:
                ...
                if file:
                    # Download file
                    mega.download_url(url, str(session_dir))
                else:
                    # Try as folder
                    mega.download_url(url, str(session_dir))
            except:
...
# ══════════════════════════════════════════════════════════════════════════════
#                       MEDIAFIRE DOWNLOADER - WORKING
# ══════════════════════════════════════════════════════════════════════════════

class MediaFireDownloader:
    """Working MediaFire downloader"""
...
class DirectDownloader:
    ...
    def download(self, url: str, session_id: str = None) -> Tuple[bool, List[Path], str]:
        ...
        try:
            ...
            # Get filename with HEAD request
...
class DownloadManager:
    ...
    async def download(self, url: str, platform: str, session_id: str = None,
        ...
        if platform == 'gofile':
            downloader = GofileDownloader(self.download_dir, progress_callback)
            return await loop.run_in_executor(None, lambda: downloader.download(url, password, session_id))
        
        elif platform == 'pixeldrain':
            downloader = PixelDrainDownloader(self.download_dir, progress_callback)
            return await loop.run_in_executor(None, lambda: downloader.download(url, session_id))
        
        elif platform == 'mega':
            downloader = MegaDownloader(self.download_dir, progress_callback)
            return await loop.run_in_executor(None, lambda: downloader.download(url, session_id))
        
        elif platform == 'mediafire':
            downloader = MediaFireDownloader(self.download_dir, progress_callback)
            return await loop.run_in_executor(None, lambda: downloader.download(url, session_id))
        
        else:
            downloader = DirectDownloader(self.download_dir, progress_callback)
            return await loop.run_in_executor(None, lambda: downloader.download(url, session_id))
        ...
        for txt_file in directory.rglob('*.txt'):
            if 'cookie' in txt_file.name.lower():
                ...
                for cookie in cookies:
                    result[cookie['domain']].append(cookie)
        return dict(result)


class TokenExtractor:
    """Extract Discord/Steam/Telegram tokens"""
    ...
    def extract_from_file(file_path: Path) -> Dict[str, List[str]]:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                # Discord
                for match in re.finditer(TokenExtractor.DISCORD_PATTERN, content):
                    tokens['discord'].append(match.group())
                
                # Steam
                for match in re.finditer(TokenExtractor.STEAM_PATTERN, content):
                    tokens['steam'].append(match.group())
                
                # Telegram
                for match in re.finditer(TokenExtractor.TELEGRAM_PATTERN, content):
                    tokens['telegram'].append(match.group())
        except:
            pass
    ...
    async def extract_from_directory(directory: Path) -> Dict[str, List[str]]:
        ...
        for file_path in token_files:
            tokens = TokenExtractor.extract_from_file(file_path)
...
class ResultPackager:
    """Package results into ZIP files"""
    ...
    def package_cc_results(cards: List[str], session_id: str) -> Path:
        ...
        zip_path = results_dir / f"CC_Results_{timestamp}.zip"
    ...
    def package_cookie_results(cookies_by_domain: Dict[str, List[Dict]], session_id: str) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        zip_path = results_dir / f"Cookies_{timestamp}.zip"
        
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            for domain, cookies in cookies_by_domain.items():
                content = "\n".join([c['raw'] for c in cookies])
    ...
    def package_token_results(tokens: Dict[str, List[str]], session_id: str) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        zip_path = results_dir / f"Tokens_{timestamp}.zip"
        
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            for token_type, token_list in tokens.items():
                if token_list:
                    zf.writestr(f"{token_type}_tokens_{len(token_list)}.txt", "\n".join(token_list))
    ...
    def package_all_files(file_list: List[Path], session_id: str, base_dir: Path) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        zip_path = results_dir / f"All_Files_{timestamp}.zip"
...
class Buttons:
    ...
    def extraction_menu(selected: List[str] = None) -> InlineKeyboardMarkup:
        ...
        def status(task):
            return "✅" if task in selected else "⬜"
...
async def check_access(message: Message) -> Tuple[bool, str]:
    ...
    # Check ban
    banned, reason = db.check_banned(user_id)
    if banned:
        await message.answer(Design.banned(reason))
        return False, ""
    
    # For demo, grant access to all
    return True, "trial"
...
async def handle_message(message: Message, state: FSMContext):
    # Check access
    granted, access_type = await check_access(message)
    if not granted:
        return
    ...
    db.create_session(session_id, message.from_user.id, url)
...
async def cb_stats(callback: CallbackQuery):
    ...
    banned, _ = db.check_banned(user_id)
    if banned:
        await callback.answer()
        return
...
async def cb_tasks(callback: CallbackQuery):
    await callback.message.edit_text(
        "📋 **TASKS**\n\n"
        f"You have {TRIAL_TASKS} trial tasks.",
        reply_markup=Buttons.back_only()
    )
    await callback.answer()
...
async def cb_proceed(callback: CallbackQuery, state: FSMContext):
    ...
    if platform == 'mega' and '#' in data.get('url', ''):
        # Mega with key in URL
...
async def cb_toggle(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_reply_markup(
        reply_markup=Buttons.extraction_menu(selected)
    )
    await callback.answer()
...
async def start_download_process(message: Message, state: FSMContext):
    ...
    # Progress callback
    async def progress_callback(filename, percent, speed, downloaded, total):
        try:
            ...
            await status_msg.edit_text(
                Design.download_progress(filename, bar, percent, speed_str, downloaded_str, total_str)
            )
        except:
            pass
    ...
    success, downloaded_files, error = await downloader.download(
        url, platform, session_id, password, progress_callback
    )
    ...
    # Extract
    await status_msg.edit_text(Design.extracting(len(downloaded_files)))
    
    extract_dir = EXTRACTS_DIR / session_id
    ...
    for file_path in downloaded_files:
        if ArchiveExtractor.is_archive(file_path):
            success, msg, extracted = await asyncio.get_event_loop().run_in_executor(
                None, ArchiveExtractor.extract, file_path, extract_dir, password
            )
            if success:
                all_extracted.extend(extracted)
            else:
                # Copy as is
                shutil.copy2(file_path, extract_dir / file_path.name)
                all_extracted.append(extract_dir / file_path.name)
        else:
            shutil.copy2(file_path, extract_dir / file_path.name)
            all_extracted.append(extract_dir / file_path.name)
    ...
    if 'cc' in selected_tasks:
        cards = await CCExtractor.extract_from_directory(extract_dir)
        if cards:
            zip_path = ResultPackager.package_cc_results(cards, session_id)
            result_files.append(('💳 Cards', zip_path, len(cards)))
            results['cc'] = len(cards)
            db.increment_stats(user_id, total_cc_found=len(cards))
    
    # Cookies
    if 'cookies' in selected_tasks:
        cookies = await CookieExtractor.extract_from_directory(extract_dir)
        total_cookies = sum(len(v) for v in cookies.values())
        if cookies:
            zip_path = ResultPackager.package_cookie_results(cookies, session_id)
            result_files.append(('🍪 Cookies', zip_path, total_cookies))
            results['cookies'] = total_cookies
            db.increment_stats(user_id, total_cookies_found=total_cookies)
    
    # Tokens
    if 'tokens' in selected_tasks:
        tokens = await TokenExtractor.extract_from_directory(extract_dir)
        total_tokens = sum(len(v) for v in tokens.values())
        if tokens:
            zip_path = ResultPackager.package_token_results(tokens, session_id)
            result_files.append(('💎 Tokens', zip_path, total_tokens))
            results['tokens'] = total_tokens
    
    # Unzip all
    if 'unzip' in selected_tasks or not selected_tasks:
        all_zip = ResultPackager.package_all_files(all_extracted, session_id, extract_dir)
    ...
    if result_files:
        ...
        for name, zip_path, count in result_files:
            if zip_path.exists() and zip_path.stat().st_size <= TELEGRAM_FILE_LIMIT:
                try:
                    await message.answer_document(
                        FSInputFile(zip_path),
                        caption=f"{name} • {count} items • {format_size(zip_path.stat().st_size)}"
                    )
                except Exception as e:
    ...
    # Cleanup
    await state.clear()
    db.log_activity(user_id, f"completed {platform}")
...
async def handle_admin_input(message: Message, state: FSMContext):
    ...
    if action == 'ban':
        try:
            user_id = int(text)
            if db.ban_user(user_id, "Banned by admin"):
